...
{
    List<scalar> reta; reta.setSize(1); reta[0]=-0.00283;
    List<scalar> retn; retn.setSize(1); retn[0]=8.6;
    List<scalar> retm; retm.setSize(1); retm[0]=0.8837;
...
void Foam::buildingMaterialModels::PorousAsphalt::update_w_C_boundary(const volScalarField& pc, volScalarField& w, volScalarField& Crel, label patchi, label patchFacei)
{
    ...
    w.boundaryField()[patchi][patchFacei] = w_tmp*48.8; 
    Crel.boundaryField()[patchi][patchFacei] = mag( C_tmp*48.8 );  
}
...
void Foam::buildingMaterialModels::PorousAsphalt::update_Krel_boundary(const volScalarField& pc, const volScalarField& w, volScalarField& Krel, label patchi, label patchFacei)
{
...
void Foam::buildingMaterialModels::PorousAsphalt::update_Kv_boundary(const volScalarField& pc, const volScalarField& w, const volScalarField& T, volScalarField& K_v, label patchi, label patchFacei)
{
    scalar rho_l = 1.0e3; 
    scalar R_v = 8.31451*1000/(18.01534); 
    ...
    scalar tmp = 1 - (w.boundaryField()[patchi][patchFacei]/48.8); 
    scalar delta = 2.61e-5 * tmp/(R_v*T.boundaryField()[patchi][patchFacei]*2*(0.89*tmp*tmp + 0.11)); 
...
void Foam::buildingMaterialModels::PorousAsphalt::update_Kpt_boundary(const volScalarField& pc, const volScalarField& w, const volScalarField& T, volScalarField& K_pt, label patchi, label patchFacei)
{
    scalar rho_l = 1.0e3; 
    scalar R_v = 8.31451*1000/(18.01534); 
    scalar L_v = 2.5e6;
    ...
    scalar tmp = 1 - (w.boundaryField()[patchi][patchFacei]/48.8); 
    scalar delta = 2.61e-5 * tmp/(R_v*T.boundaryField()[patchi][patchFacei]*2*(0.89*tmp*tmp + 0.11)); // Water vapour diffusion coefficient "for concrete" [s]
...
void Foam::buildingMaterialModels::PorousAsphalt::update_lambda_boundary(const volScalarField& w, volScalarField& lambda, label patchi, label patchFacei)
{
