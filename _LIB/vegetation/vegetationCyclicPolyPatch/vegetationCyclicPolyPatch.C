...
#include "vegetationCyclicPolyPatch.H"
...
namespace Foam
{
    defineTypeNameAndDebug(vegetationCyclicPolyPatch, 0);

    addToRunTimeSelectionTable(polyPatch, vegetationCyclicPolyPatch, word);
    addToRunTimeSelectionTable(polyPatch, vegetationCyclicPolyPatch, dictionary);
}


// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * //

Foam::label Foam::vegetationCyclicPolyPatch::findMaxArea
(
...
void Foam::vegetationCyclicPolyPatch::calcTransforms()
{
    if (size())
    {
        // Half0
        const vegetationCyclicPolyPatch& half0 = *this;
        ...
        // Half1
        const vegetationCyclicPolyPatch& half1 = neighbPatch();
...
)
{
    if (debug && owner())
    {
        fileName casePath(boundaryMesh().mesh().time().path());
        {
            ...
            Pout<< "vegetationCyclicPolyPatch::calcTransforms : Writing " << name()
        ...
        const vegetationCyclicPolyPatch& half1 = neighbPatch();
        {
            ...
            Pout<< "vegetationCyclicPolyPatch::calcTransforms : Writing " << half1.name()
        ...
        }
        {
            ...
            label vertI = 0;
            Pout<< "vegetationCyclicPolyPatch::calcTransforms :"
    ...
    if (half0Ctrs.size() != half1Ctrs.size())
    {
        FatalErrorIn
        (
            "vegetationCyclicPolyPatch::calcTransforms()"
        )   << "For patch " << name()
...
void Foam::vegetationCyclicPolyPatch::getCentresAndAnchors
(
...
) const
{
    ...
    if (debug)
    {
        Pout<< "vegetationCyclicPolyPatch::getCentresAndAnchors :"
            << " patch:" << name() << nl
...
Foam::vector Foam::vegetationCyclicPolyPatch::findFaceMaxRadius
(
...
Foam::vegetationCyclicPolyPatch::vegetationCyclicPolyPatch
(
    const word& name,
    const label size,
    const label start,
    ...
    coupledEdgesPtr_(NULL)
{
    if (neighbPatchName_ == word::null && !coupleGroup_.valid())
    {
        FatalIOErrorIn
        (
            "vegetationCyclicPolyPatch::vegetationCyclicPolyPatch\n"
            "(\n"
            "    const word& name,\n"
            "    const dictionary& dict,\n"
            "    const label index,\n"
    ...
    {
        FatalIOErrorIn("vegetationCyclicPolyPatch::vegetationCyclicPolyPatch(..)", dict)
...
Foam::vegetationCyclicPolyPatch::vegetationCyclicPolyPatch
(
    const vegetationCyclicPolyPatch& pp,
    const polyBoundaryMesh& bm
)
:
    ...
    coupledEdgesPtr_(NULL)
{
    if (neighbName == name())
    {
        FatalErrorIn("vegetationCyclicPolyPatch::vegetationCyclicPolyPatch(..)")
...
const Foam::word& Foam::vegetationCyclicPolyPatch::neighbPatchName() const
{
...
Foam::label Foam::vegetationCyclicPolyPatch::neighbPatchID() const
{
    if (neighbPatchID_ == -1)
    {
        ...
        if (neighbPatchID_ == -1)
        {
            FatalErrorIn("vegetationCyclicPolyPatch::neighbPatchID() const")
        ...
        const vegetationCyclicPolyPatch& nbrPatch = refCast<const vegetationCyclicPolyPatch>
        (
        ...
        {
            WarningIn("vegetationCyclicPolyPatch::neighbPatchID() const")
                << "Patch " << name()
...
void Foam::vegetationCyclicPolyPatch::transformPosition(pointField& l) const
{
    if (!parallel())
    {
        if (transform() == ROTATIONAL)
...
void Foam::vegetationCyclicPolyPatch::transformPosition(point& l, const label facei) const
{
    if (!parallel())
    {
        const tensor& T =
...
void Foam::vegetationCyclicPolyPatch::initGeometry(PstreamBuffers& pBufs)
{
    polyPatch::initGeometry(pBufs);
}


void Foam::vegetationCyclicPolyPatch::initGeometry
(
...
void Foam::vegetationCyclicPolyPatch::calcGeometry
(
    const primitivePatch& referPatch,
...
void Foam::vegetationCyclicPolyPatch::calcGeometry(PstreamBuffers& pBufs)
{
    calcGeometry
    (
        *this,
...
void Foam::vegetationCyclicPolyPatch::initMovePoints
(
...
void Foam::vegetationCyclicPolyPatch::movePoints
(
    PstreamBuffers& pBufs,
    const pointField& p
)
...
void Foam::vegetationCyclicPolyPatch::initUpdateMesh(PstreamBuffers& pBufs)
{
    polyPatch::initUpdateMesh(pBufs);
}


void Foam::vegetationCyclicPolyPatch::updateMesh(PstreamBuffers& pBufs)
{
...
const Foam::edgeList& Foam::vegetationCyclicPolyPatch::coupledPoints() const
{
...
const Foam::edgeList& Foam::vegetationCyclicPolyPatch::coupledEdges() const
{
    if (!coupledEdgesPtr_)
    {
        ...
        const vegetationCyclicPolyPatch& neighbPatch = this->neighbPatch();
        ...
        forAll(coupledEdges, i)
        {
            ...
            if (e[0] < 0 || e[1] < 0)
            {
                FatalErrorIn("vegetationCyclicPolyPatch::coupledEdges() const")
...
void Foam::vegetationCyclicPolyPatch::initOrder
(
    PstreamBuffers&,
...
bool Foam::vegetationCyclicPolyPatch::order
(
    PstreamBuffers& pBufs,
    const primitivePatch& pp,
    labelList& faceMap,
...
) const
{
    ...
    else
    {
        ...
        if (!matchedAll || debug)
        {
            ...
            Pout<< "vegetationCyclicPolyPatch::order : Writing neighbour"
                << " faces to OBJ file " << nm0 << endl;
            ...
            Pout<< "vegetationCyclicPolyPatch::order : Writing my"
                << " faces to OBJ file " << nm1 << endl;
            ...
            Pout<< "vegetationCyclicPolyPatch::order : "
        ...
        if (!matchedAll)
        {
            SeriousErrorIn
            (
                "vegetationCyclicPolyPatch::order"
        ...
        forAll(faceMap, oldFaceI)
        {
            ...
            if (rotation[newFaceI] == -1)
            {
                SeriousErrorIn
                (
                    "vegetationCyclicPolyPatch::order(const primitivePatch&"
...
void Foam::vegetationCyclicPolyPatch::write(Ostream& os) const
{
