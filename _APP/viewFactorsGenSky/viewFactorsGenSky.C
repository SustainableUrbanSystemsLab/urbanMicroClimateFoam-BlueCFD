...
int main(int argc, char *argv[])
{
    ...
    scalarList rowSumViewFactorPatch
    (
        totalPatches,
        0.0
    );		
	forAll(viewFactorsPatches, i)
	{
		...
		forAll(viewFactorsPatches, i)
		{
			...
			rowSumViewFactorPatch[patchI] += sumViewFactorPatch[patchI][patchJ]/patchArea[patchI];
		}
	}
	forAll(localCoarseSf, coarseFaceI)
	{
		...
		forAll (visCoarseFaces, visCoarseFaceI)
		{
			...
			if ((fromPatchId != 2) && (toPatchId == TOP_PATCH_ID))
			{
				...
				scalar A = rowSumViewFactorPatch[fromPatchId]; // sum of F (from this patch to all)
